...
class RobustSE3PoseGraph:
    """Maximum-information forest plus robust loop validation in full SE(3)."""
    ...
    def optimize(
        ...
        *,
        ...
        if translation_gate_m <= 0 or rotation_gate_rad <= 0:
        ...
        for index, constraint in enumerate(self._constraints):
            ...
            trace = float(np.trace(error[:3, :3]))
            rotation_error = float(np.arccos(np.clip((trace - 1.0) / 2.0, -1.0, 1.0)))
            translation_residuals.append(translation_error)
            rotation_residuals.append(rotation_error)
