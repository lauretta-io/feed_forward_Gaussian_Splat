...
from ...encoder.mvsplat.costvolume.conversions import depth_to_relative_disparity


def _load_diff_gaussian_rasterization():
    try:
        ...
        raise ImportError(
            "MVSplat's legacy CUDA decoder requires "
            "`diff_gaussian_rasterization`. Install the optional MVSplat "
            "rasterizer dependency before using `model/decoder=mvsplat_splatting_cuda`."
        ) from exc
...
def render_cuda(
    ...
    GaussianRasterizationSettings, GaussianRasterizer = (
        _load_diff_gaussian_rasterization()
    )
